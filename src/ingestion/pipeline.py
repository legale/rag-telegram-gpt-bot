...
class IngestionPipeline:
    ...
    def clear_stage2(self) -> int:
        """Clear stage2: L1 clustering results (topics_l1)."""
        syslog2(LOG_INFO, "clearing stage2: topics_l1")
        deleted = self.db.clear_topics_l1()
        syslog2(LOG_INFO, "stage2 cleared", deleted=deleted)
        return deleted

    def clear_stage2b(self) -> int:
        """Clear stage2b: topic_l1_id assignments in chunks."""
        syslog2(LOG_INFO, "clearing stage2b: topic_l1_id assignments")
        updated = self.db.clear_chunk_topic_l1_assignments()
        syslog2(LOG_INFO, "stage2b cleared", updated=updated)
        return updated

    def clear_stage3(self) -> int:
        """Clear stage3: topic names for L1 topics (will be regenerated by name_topics)."""
        # Stage3 doesn't have separate data to clear - names are stored in topics_l1.title/descr
        # This stage just regenerates names, so clearing is a no-op
        syslog2(LOG_INFO, "clearing stage3: no data to clear (names stored in topics_l1)")
        return 0
    ...
    def run_stage2(self, **clustering_params):
        ...
        # Store assignments for stage2b
        self._stage2_assignments = assignments

    def run_stage2b(self):
        """Run stage2b: assign topic_l1_id to chunks based on clustering results."""
        if not hasattr(self, '_stage2_assignments'):
            raise ValueError("stage2b requires stage2 to be run first")
        ...
        clusterer._l1_topic_assignments = self._stage2_assignments
        clusterer.assign_l1_topics_to_chunks()
        
        # Clear assignments after use
        delattr(self, '_stage2_assignments')
    ...
    def run_all(self, file_path: str, model: Optional[str] = None, batch_size: int = 128, **clustering_params):
        ...
        print("\nRunning stage2b: Assign topics to chunks...")
        self.run_stage2b()
